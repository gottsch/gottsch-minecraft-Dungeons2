...
/** The {@code colonnade} column footprint, at {@code spacing} apart and {@code inset} from the wall. */
public record ColonnadePillarLayout(int spacing, int inset) implements PillarLayoutPattern {

    public static final String NAME = "colonnade";

    /** The default rhythm, for a callers-with-no-JSON convenience. */
    public ColonnadePillarLayout() {
        this(GridPillarPatternProvider.DEFAULT_SPACING, GridPillarPatternProvider.DEFAULT_INSET);
    }

    public static final MapCodec<ColonnadePillarLayout> CODEC = PillarLayoutPattern.RHYTHM(
            ColonnadePillarLayout::new, ColonnadePillarLayout::spacing, ColonnadePillarLayout::inset);
    ...
    public IPillarPatternProvider provider() {
        return new ColonnadePillarPatternProvider(spacing, inset);
    }
}
