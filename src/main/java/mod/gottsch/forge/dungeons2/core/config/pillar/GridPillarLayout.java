...
/** The {@code grid} column footprint, at {@code spacing} apart and {@code inset} from the wall. */
public record GridPillarLayout(int spacing, int inset) implements PillarLayoutPattern {

    public static final String NAME = "grid";

    /** The default rhythm, for a callers-with-no-JSON convenience. */
    public GridPillarLayout() {
        this(GridPillarPatternProvider.DEFAULT_SPACING, GridPillarPatternProvider.DEFAULT_INSET);
    }

    public static final MapCodec<GridPillarLayout> CODEC = PillarLayoutPattern.RHYTHM(
            GridPillarLayout::new, GridPillarLayout::spacing, GridPillarLayout::inset);
    ...
    public IPillarPatternProvider provider() {
        return new GridPillarPatternProvider(spacing, inset);
    }
}
